...
const results = [];
try {
  const get = (path, cookie = '', method = 'GET', host = 'barbacue.cog.ia.br') => fetch(`https://${host}${path}`, { method, redirect: 'manual', headers: { Cookie: cookie }, signal: AbortSignal.timeout(120000) });
  ...
  const payload = `0|admin|${Date.now() + 300000}`;
  ...
  try {
    ...
    if (rows[0]) {
      const p = `${rows[0].id}|${rows[0].role}|${Date.now() + 300000}`;
  ...
  writeFileSync(`${backup}/public-verification.json`, JSON.stringify({ success: true, results }, null, 2));
